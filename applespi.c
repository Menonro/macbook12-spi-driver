...
{
       efi_guid_t guid = EFI_BL_LEVEL_GUID;
       unsigned long len = sizeof(u16);
       u16 data = 0;
       u32 attr;
       efi_status_t status;
       int err;

       status = efivar_get_variable((efi_char16_t *)EFI_BL_LEVEL_NAME, &guid,
                                   &attr, &len, &data);
                                   ...
                                   unsigned int level)
{
       ...
       status = efivar_set_variable((efi_char16_t *)EFI_BL_LEVEL_NAME, &guid,
                                    attr, sizeof(data), &data);
